...
class Block:
    ...
    def get_seqs(self):
        ...
        best = []
        for ptr in tqdm.tqdm(self.pointers):
            offset = ptr["idx"] - self.seqs["idx"][0] if not self.seqs.empty else 0
    ...
    def create_ptr_table(self):
        print("GO")
        # pointers = pd.DataFrame(self.pointers)
        # seqs = pd.DataFrame(self.seqs, columns=["idx", "seq"])

        # pointer_table = pd.merge_asof(pointers, seqs, on="idx")
