...
class Block:
    def __init__(self, tid, table, address):
        self.tid = tid
        self.table = table
        ...
        self.is_empty = True * len(self.seqs)
    ...
    def get_seqs(self):
        ...
        # Find best offset to match max amount of ptrs to seqs
        best = []
    ...
    def create_ptr_table(self):
        print("-------------------------Block ID: ", self.tid)
        for p in self.pointers:
            if p["val"] >= 0 and p["val"] < len(self.table):

                for s in self.seqs:
                    if self.table.index(s) == p["val"]:
                        p["addr"] = hex(self.address + p["val"])
                        p["seq"] = s
                        p["decoded"] = bytes.fromhex(s).decode("shift-jis", "ignore")
        ...
        # if len(self.pointers) == len(self.seqs):
            # self.solved = True
            # print("Block has been solved!")
...
if __name__=="__main__":
    get_ptr_tables(os.path.join(path, config.BIN_PATH))
